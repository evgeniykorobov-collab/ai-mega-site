...
// 2. Replace ai-solutions benefits
const oldSolutionsStartStr = `        <div id="ai-solutions" className="flow-container" style={{ paddingTop: '50px' }}>`;
const oldSolutionsStart = code.indexOf(oldSolutionsStartStr);
const nextSectionStart = code.indexOf('</section>', oldSolutionsStart);
// Actually, ai-solutions is a div, the section closes after it!
// Let's use the comment block to locate boundaries precisely.
// In the current file:
//         <div id="ai-solutions" className="flow-container" ...>
//            ...
//         </div>
//       </section>

// Wait, the safest way is substring replacement by finding the `        <div id="ai-solutions" className="flow-container"` and `        </div>\n      </section>`.
let replacement = `        <div id="ai-solutions" className="flow-container" style={{ paddingTop: '50px' }}>
          ...
          {[
            { 
               ...
               fullDesc: "Система не знает усталости, не допускает опечаток и обрабатывает запросы с точностью совершенной машины.",
            ...
            { 
               ...
               fullDesc: "Все звонки и переписки автоматически анализируются. Сводки готовы в реальном времени, а не в конце месяца.",
            ...
            { 
               ...
               fullDesc: "Бизнес вырастет в 10 раз? Вам не придется искать помещения и выстраивать новые отделы.",
              ...
              >
                  <div className="flow-content terminal-panel" style={{ position: 'relative' }}>
                      ...
                      <div className="terminal-body" style={{ position: 'relative', zIndex: 1, textShadow: '0 0 10px rgba(0,0,0,0.9), 0 0 20px rgba(0,0,0,0.9)', minHeight: '260px', padding: '40px 30px' }}>
                          ...
                          <p className="mono" style={{ color: '#fff', fontSize: '0.9rem', marginTop: '15px', opacity: 1, lineHeight: '1.5', textShadow: '0 0 5px rgba(0,240,255,0.7)' }}>[ РЕЗУЛЬТАТ ]: {benefit.result}</p>
                      </div>
                  </div>
